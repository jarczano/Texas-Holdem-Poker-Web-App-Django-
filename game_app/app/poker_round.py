import random

from channels.layers import get_channel_layer
from asgiref.sync import async_to_sync

from game_app.app.auction import auction
from game_app.app.poker_score import players_score
from game_app.app.split_pot import one_player_win, change_players_positions, split_pot
from game_app.app.player_class import Player
from game_app.app.setting import BB, SB, show_game


def poker_round(player_id):
    """
    This function works as generator. Play one round of dealing cards.
    Sends information to the client about situation on the table.
    :param player_id:
    :return: yield 'wait_for_player_decision' when is turn for client.
    """

    channel_layer = get_channel_layer()

    player_list_chair = Player.player_list_chair
    player_list = Player.player_list

    async_to_sync(channel_layer.group_send)(
        f'{player_id}',{
            'type': 'message_decision',
            'message': 'New round'})

    # Take blinds from players
    if player_list[-1].stack > BB:
        player_list[-1].blind(BB)

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'message_decision',
                'message': player_list[-1].name + ' big blind ' + str(BB) + "$"})
    else:
        player_list[-1].blind(player_list[-1].stack)
        player_list[-1].allin()

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'message_decision',
                'message': player_list[-1].name + ' big blind ' + player_list[-1].input_stack + "$"})

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'message_decision',
                'message': player_list[-1].name + ' short stack all-in ' + player_list[-1].input_stack})

    if player_list[-2].stack > SB:
        player_list[-2].blind(SB)

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'message_decision',
                'message': player_list[-2].name + ' small blind ' + str(SB) + "$"})

    else:
        player_list[-2].blind(player_list[-2].stack)
        player_list[-2].allin()

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'message_decision',
                'message': player_list[-2].name + ' small blind ' + player_list[-2].input_stack + "$"})

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'message_decision',
                'message': player_list[-2].name + ' short stack all-in ' + player_list[-2].input_stack})

    # send info about blind to the client
    for player in player_list_chair:

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'update_bet',
                'message': [player.name, player.input_stack]})

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'update_stack',
                'message': [player.name, player.stack]})

    # send info about pot
    pot = sum([player.input_stack for player in player_list])

    async_to_sync(channel_layer.group_send)(
        f'{player_id}', {
            'type': 'update_pot',
            'message': [pot]})

    if show_game:
        print(player_list[-1].name, 'BB')
        print(player_list[-2].name, 'SB')

    # Create a deck of cards
    deck = ['2C', '3C', '4C', '5C', '6C', '7C', '8C', '9C', 'TC', 'JC', 'QC', 'KC', 'AC', '2S', '3S', '4S', '5S', '6S',
            '7S', '8S', '9S', 'TS', 'JS', 'QS', 'KS', 'AS', '2H', '3H', '4H', '5H', '6H', '7H', '8H', '9H', 'TH', 'JH',
            'QH', 'KH', 'AH', '2D', '3D', '4D', '5D', '6D', '7D', '8D', '9D', 'TD', 'JD', 'QD', 'KD', 'AD']

    # Deal the cards to the player
    for player in player_list_chair:
        player.cards = random.sample(deck, 2)
        [deck.remove(player.cards[i]) for i in range(2)]
        # send player card to the client
        if player.kind == 'human':

            async_to_sync(channel_layer.group_send)(
                f'{player_id}', {
                    'type': 'deal_player_cards',
                    'message': player.cards})
        else:

            async_to_sync(channel_layer.group_send)(
                f'{player_id}', {
                    'type': 'deal_opponent_cards',
                    'message': ''})

    if show_game:
        for player in player_list:
            print(player.name, player.cards)

    # Check how many players are in the game and all-in
    number_live_players = sum([player.live for player in player_list])  # 1
    number_allin_players = sum([player.alin for player in player_list])  # 0

    if number_live_players > 1:
        # First auction

        if show_game:
            print("Preflop")

        yield from auction(player_id)

        # reset bet auction
        for player in player_list:
            async_to_sync(channel_layer.group_send)(
                f'{player_id}', {
                    'type': 'update_bet',
                    'message': [player.name, 0]})

    # Check how many players are in the game and all-in
    number_live_players = sum([player.live for player in player_list])  # 1
    number_allin_players = sum([player.alin for player in player_list]) # 0

    # change order decision player
    if len(player_list_chair) == 2:
        shift_decision = -1
    else:
        shift_decision = -2

    # If there is only one player left in the game, he wins
    if number_live_players + number_allin_players == 1:

        one_player_win(player_id)
        # end of round

        # hide opponent decision

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'hide_opponent_decision',
                'message': ''})
    else:
        # Flop

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'hide_opponent_decision',
                'message': ''})
        flop = random.sample(deck, 3)
        [deck.remove(flop[i]) for i in range(3)]
        common_cards = flop

        # send flop cards to client

        async_to_sync(channel_layer.group_send)(
            f'{player_id}', {
                'type': 'deal_flop_cards',
                'message': common_cards})

        if show_game:
            print("Flop cards: {}".format(flop))

        # Change order decision players
        change_players_positions(shift_decision)

        if number_live_players > 1:

            # Second auction
            yield from auction(player_id, common_cards)

            # reset auction bet
            for player in player_list:
                async_to_sync(channel_layer.group_send)(
                    f'{player_id}', {
                        'type': 'update_bet',
                        'message':[player.name, 0]})

        # Check how many players are in the game and all-in
        number_live_players = sum([player.live for player in player_list])
        number_allin_players = sum([player.alin for player in player_list])

        # If there is only one player left in the game, he wins
        if number_live_players + number_allin_players == 1:

            one_player_win(player_id)

            # end of round

            # hide opponent decision

            async_to_sync(channel_layer.group_send)(
                f'{player_id}', {
                    'type': 'hide_opponent_decision',
                    'message': ''})
            # Return to the original position
            change_players_positions(shift_decision)
        else:

            async_to_sync(channel_layer.group_send)(
                f'{player_id}', {
                    'type': 'hide_opponent_decision',
                    'message': ''})
            # Deal the cards to the turn
            turn = random.sample(deck, 1)
            deck.remove(turn[0])
            common_cards = flop + turn

            # send turn card to client

            async_to_sync(channel_layer.group_send)(
                f'{player_id}', {
                    'type': 'deal_turn_card',
                    'message': turn})

            if show_game:
                print("Turn cards: {}".format(common_cards))

            if number_live_players > 1:
                # Third auction
                yield from auction(player_id, common_cards)

                # reset auction bet
                for player in player_list:
                    async_to_sync(channel_layer.group_send)(
                        f'{player_id}', {
                            'type': 'update_bet',
                            'message': [player.name, 0]})

            # Check how many players are in the game and all-in
            number_live_players = sum([player.live for player in player_list])
            number_allin_players = sum([player.alin for player in player_list])

            # If there is only one player left in the game, he wins
            if number_live_players + number_allin_players == 1:
                one_player_win(player_id)
                
                # end of round

                # hide opponent decision

                async_to_sync(channel_layer.group_send)(
                    f'{player_id}', {
                        'type': 'hide_opponent_decision',
                        'message': ''})
                change_players_positions(shift_decision)
            else:

                async_to_sync(channel_layer.group_send)(
                    f'{player_id}', {
                        'type': 'hide_opponent_decision',
                        'message': ''})
                # Deal the cards to the river
                river = random.sample(deck, 1)
                deck.remove(river[0])
                common_cards += river

                async_to_sync(channel_layer.group_send)(
                    f'{player_id}', {
                        'type': 'deal_river_card',
                        'message': river})
                # send river card to client

                if show_game:
                    print("River cards: {}".format(common_cards))

                if number_live_players > 1:
                    # Last auction
                    yield from auction(player_id, common_cards)

                        # reset auction bet
                    for player in player_list:
                        async_to_sync(channel_layer.group_send)(
                            f'{player_id}', {
                                'type': 'update_bet',
                                'message': [player.name, 0]})

                # Check how many players are in the game and all-in
                number_live_players = sum([player.live for player in player_list])
                number_allin_players = sum([player.alin for player in player_list])

                # If there is only one player left in the game, he wins
                if number_live_players + number_allin_players == 1:

                    # Return to the original position
                    one_player_win(player_id)

                    # end of round

                     # hide opponent decision
                    async_to_sync(channel_layer.group_send)(
                        f'{player_id}', {
                            'type': 'hide_opponent_decision',
                            'message': ''})
                    change_players_positions(shift_decision)
                else:

                    # Send message to show down client
                    for player in player_list:
                        if player.kind != 'human':
                            async_to_sync(channel_layer.group_send)(
                                f'{player_id}', {
                                    'type': 'show_down',
                                    'message': player.cards})
                            print("SOHW DOWN MESSAGE")

                    # Calculate score players
                    players_score(player_list_chair, common_cards)

                    # Split pot
                    split_pot(player_id)

                    # end of round
                    # Return players to the original position
                    change_players_positions(shift_decision)

